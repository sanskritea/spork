...
Adapted from serial driver for ESP300 and Ethernet driver for ESP 

Sanskriti Chitransh, 2024-May-10

Once I use the socket and ip address to make an ethernet connection to the ESP, I need to find a way to separate the three axes and communicate with the controller
...
class ESP(object):
  ...
  def __init__(self):
    ...
    #                          parity='N',
    #                          rtscts=1)
    self.ser = serial.serial_for_url('socket://192.168.1.90:5001')
  ...
  def write(self, string, axis=None):
    ...
    self.ser.write(((str(axis) if axis is not None else "") + string + "\r").encode('utf_8'))
