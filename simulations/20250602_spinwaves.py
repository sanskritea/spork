...
class SpinWaveCalculator:
    ...
    def multi_para_diag(self, h_NV_array, omega_H, q_table, phi_table, N_max, n_processes=None):
        ...
        # Parallel computation with larger chunk size for better load balancing
        ...
        with Pool(n_processes) as pool:
            results = list(tqdm(
                pool.imap(self.calculate_single_point, args_list, chunksize=chunk_size),
                total=len(args_list),
    ...
    def create_interpolators_vectorized(self, q_table, phi_table, results):
        """Create vectorized interpolation functions for better performance"""
        ...
        # Create more efficient RegularGridInterpolator objects
        interpolators = {
            'omega': [],
            'coupling_plus': [],
            'coupling_minus': [],
        ...
        # Use RegularGridInterpolator for better performance
        q_coords = np.unique(q_grid.ravel())
        phi_coords = np.unique(phi_grid.ravel())
        
        for s in range(N_max):
            # Omega interpolator - reshape data for RegularGridInterpolator
            omega_data = results['omega_bdg_table'][:, :, s]
            interpolators['omega'].append(
                RegularGridInterpolator((q_table, phi_extended), omega_data, 
                                      method='linear', bounds_error=False, fill_value=0)
            )
            
            # Coupling interpolators for each height
            for h_idx in range(num_h_NV):
                if len(interpolators['coupling_plus']) <= h_idx:
                    for coupling_type in ['plus', 'minus', 'z']:
                        interpolators[f'coupling_{coupling_type}'].append([])
                
                for coupling_type in ['plus', 'minus', 'z']:
                    coupling_data = results[f'coupling_{coupling_type}_table'][:, :, h_idx, s]
                    interpolators[f'coupling_{coupling_type}'][h_idx].append(
                        RegularGridInterpolator((q_table, phi_extended), coupling_data,
                                              method='linear', bounds_error=False, fill_value=0+0j)
                    )
        
        return interpolators
    
    def gamma_calculation_worker(self, args):
        """Worker function for parallel gamma calculation"""
        (s, q_indices, phi_indices, q_table_fine, delta_q, delta_phi, 
         eta_small, omega_target_L, omega_target_U, interpolators, 
         num_h_NV, calc_params) = args
        
        omega_M, omega_d, L, N_bose_func = calc_params
        
        # Initialize local results
        local_DOS_L = 0
        local_DOS_U = 0
        local_gamma_L_Hz = np.zeros(num_h_NV)
        local_gamma_U_Hz = np.zeros(num_h_NV)
        
        # Process assigned q and phi indices
        for i in q_indices:
            for j in phi_indices:
                q = q_table_fine[i]
                phi = 2 * np.pi * j / len(phi_indices)
                
                # Get interpolated values using vectorized approach
                try:
                    omega_val = interpolators['omega'][s]([q, phi])[0]
                    if np.isnan(omega_val) or omega_val <= 0:
                        continue
                    
                    # Calculate Lorentzian factors
                    lorentz_L = (eta_small / np.pi) / (eta_small**2 + 
                                (omega_M * omega_val - omega_target_L)**2)
                    lorentz_U = (eta_small / np.pi) / (eta_small**2 + 
                                (omega_M * omega_val - omega_target_U)**2)
                    
                    # Density of states contribution
                    dos_factor = (1 / L**2) * delta_phi / (2*np.pi)**2 * delta_q[i] * q
                    local_DOS_L += dos_factor * lorentz_L
                    local_DOS_U += dos_factor * lorentz_U
                    
                    # Dissipation rate calculation
                    bose_factor = 2 * N_bose_func(omega_M * omega_val) + 1
                    gamma_prefactor = ((2*np.pi)**2 * omega_M * omega_d * 
                    ...
                    for h_idx in range(num_h_NV):
                        # Get coupling values
                        v_plus = interpolators['coupling_plus'][h_idx][s]([q, phi])[0]
                        v_minus = interpolators['coupling_minus'][h_idx][s]([q, phi])[0]
                        
                        if not (np.isnan(v_plus) or np.isnan(v_minus)):
                            local_gamma_L_Hz[h_idx] += (gamma_prefactor * 
                                                       np.abs(v_plus)**2 * lorentz_L)
                            local_gamma_U_Hz[h_idx] += (gamma_prefactor * 
                                                       np.abs(v_minus)**2 * lorentz_U)
                except Exception as e:
                    continue  # Skip problematic points
        
        return local_DOS_L, local_DOS_U, local_gamma_L_Hz, local_gamma_U_Hz
    
    def calculate_gamma_values_parallel(self, eta_small, N_Q_est, N_phi, H0, interpolators, 
                                      num_h_NV, N_max, n_processes=None):
        """Parallelized calculation of dissipation rates Gamma_L and Gamma_U"""
        if n_processes is None:
            n_processes = cpu_count()
        ...
        calc_params = (self.omega_M, self.omega_d, self.L, self.N_bose)
        
        # Divide work among processes for each mode
        print(f"Using {n_processes} processes for gamma calculation")
        
        for s in range(N_max):
            print(f"Processing mode {s+1}/{N_max} with parallel computation")
            
            # Divide q and phi indices among processes
            q_indices_split = np.array_split(range(len(delta_q)), n_processes)
            phi_indices_split = np.array_split(range(N_phi), n_processes)
            
            # Prepare arguments for parallel processing
            args_list = []
            for proc_idx in range(n_processes):
                args_list.append((
                    s, q_indices_split[proc_idx], phi_indices_split[proc_idx],
                    q_table_fine, delta_q, delta_phi, eta_small,
                    omega_target_L, omega_target_U, interpolators,
                    num_h_NV, calc_params
                ))
            
            # Parallel computation
            with Pool(n_processes) as pool:
                results = pool.map(self.gamma_calculation_worker, args_list)
            
            # Aggregate results
            for local_DOS_L, local_DOS_U, local_gamma_L_Hz, local_gamma_U_Hz in results:
                DOS_L += local_DOS_L
                DOS_U += local_DOS_U
                gamma_L_Hz_array += local_gamma_L_Hz
                gamma_U_Hz_array += local_gamma_U_Hz
    ...
    def calculate_gamma_values(self, eta_small, N_Q_est, N_phi, H0, interpolators, num_h_NV, N_max):
        """Wrapper to choose between parallel and serial gamma calculation"""
        return self.calculate_gamma_values_parallel(eta_small, N_Q_est, N_phi, H0, 
                                                   interpolators, num_h_NV, N_max)

def main():
    """Main execution function with enhanced parallelization"""
    ...
    cpu_cores = cpu_count()
    print(f"System info: {cpu_cores} CPU cores, {available_memory:.1f} GB available RAM")
    
    # Parameters for calculation
    h_NV_array = np.array([0.4]) #, 0.5, 0.6, 0.7])  # NV heights in μm
    H0_array = np.array([81, 81.5, 82, 82.5, 83])
    ...
    for i, H0 in enumerate(tqdm(H0_array, desc="Processing magnetic fields")):
        ...
        # Parallel computation for Hamiltonian diagonalization
        results = calc.multi_para_diag(h_NV_array, omega_H, q_table, phi_table, N_max)
        
        # Create optimized interpolators
        interpolators = calc.create_interpolators_vectorized(q_table, phi_table, results)
        
        # Calculate dissipation rates with parallel processing
        eta_small = 0.003
        N_Q_est = 400
        N_phi_fine = 720
