...
import experiments.ThresholdVsTime as ThresholdVsTime
from guis.guiElements_general import AutoSaveWidget
from guis.guiElements_general import flexSave

class ThresholdVsTime_Widget(QtWidgets.QWidget):

	def __init__(self):
		super().__init__()

		self.setWindowTitle('ThresholdVsTime')

		self.params_widget = ParamsWidget({
            'datasetname': {
                'display_text': 'Dataset Name',
                'widget': QtWidgets.QLineEdit('ThresholdVsTime'),
            },
        })
	...
	def runClicked(self):
	    ...
	    reload(ThresholdVsTime)

	    # create an instance of the ODMR class that implements the experimental logic.
	    ThresholdVsTimeMeas = ThresholdVsTime.ThresholdVsTime_Measurement()

	    # run the sweep function in a new thread
	    self.sweepProc.run(
	        ThresholdVsTimeMeas.auxoutvstime,
...
class ThresholdVsTimePlotWidget(LinePlotWidget):
    ...
    def setup(self):
        self.sink = DataSink('ThresholdVsTime')
        self.sink.__enter__() 
        self.add_plot('Threshold output')
    ...
    def update(self):
        ...
        # update the plot
        self.set_data('Threshold output', self.sink.datasets['time'], self.sink.datasets['AUX'])
