...
import experiments.AOMLag as AOMLag


class AOMLag_Widget(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.params_widget = ParamsWidget({
            ...
            'datasetName': {
                ...
                'widget': QtWidgets.QLineEdit('AOM Lag'),
            },
            ...
            'init_time': {
                ...
                'widget': SpinBox(
                    value=2e-6, # 1-2us at saturation, can go longer
                    suffix='s',
    ...
    def runClicked(self):
        ...
        AOMLagMeas = AOMLag.AOM_Lag_Meas()
...
class AOMLagPlotWidget(LinePlotWidget):

    def __init__(self):
        title = 'AOM Lag'
        super().__init__(title=title, xlabel='Taus (ns)', ylabel='PL')


    def setup(self):
        self.sink = DataSink('AOM Lag')
        self.sink.__enter__() 
        self.add_plot('Counts')
        # self.add_plot('Other Counts')
    ...
    def update(self):
        ...
        self.set_data('Counts', tau_list, [np.mean(self.sink.datasets['PLcounts'][tau]) for tau in tau_list])
        # self.set_data('Other Counts', tau_list, [np.mean(self.sink.datasets['OtherPLcounts'][tau]) for tau in tau_list])
